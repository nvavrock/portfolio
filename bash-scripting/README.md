...
- Handout: [`docs/TWN_Bash_Scripting_Handout.pdf`](./docs/TWN_Bash_Scripting_Handout.pdf)
- My notes: [`docs/lessons_learned.pdf`](./docs/lessons_learned.pdf)

I followed the video and the handout end-to-end, built the log-analysis script, and then used **Cursor** to take the rough handout-style script and turn it into a polished, production-quality version. The two scripts live side by side in [`scripts/`](./scripts) so the difference is easy to read.
...
├── README.md                       # this file
├── .gitignore
├── log_analysis_report.txt         # generated by the cursorized script (gitignored)
├── docs/
│   ├── TWN_Bash_Scripting_Handout.pdf   # original handout from the tutorial
│   └── lessons_learned.pdf              # my own takeaways from the project
├── scripts/
│   ├── analyze_logs.sh                  # original, handout-style script
│   └── analyze_logs_cursorized.sh       # cleaned-up version produced with Cursor
└── sample_logs/
    ├── application.log
    └── system.log
```
...
These topics from the handout are exercised by the scripts in [`scripts/`](./scripts):

| Handout section | How it shows up here |
|---|---|
| Sample Log Files | [`sample_logs/application.log`](./sample_logs/application.log), [`sample_logs/system.log`](./sample_logs/system.log) |
...
## Log analysis scripts (`scripts/`)

| File | Role |
|------|------|
| [`scripts/analyze_logs.sh`](./scripts/analyze_logs.sh) | Original handout-style script — what I had after working through the video. |
| [`scripts/analyze_logs_cursorized.sh`](./scripts/analyze_logs_cursorized.sh) | "Cursorized" version — refined inside Cursor to fix bugs, add a real report file, and harden the script. |
...
```bash
bash ~/portfolio/bash_scripting/scripts/analyze_logs_cursorized.sh
```

The original handout-style version (kept unchanged for comparison):

```bash
bash ~/portfolio/bash_scripting/scripts/analyze_logs.sh
```
